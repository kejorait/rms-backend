...
from helper import printSelenium
    ...
    def getBillDtlByTable(self, request, db):
        ...
        try:
            ...
            if bill:
                ...
                table_session_query = (db.query(TableSession.created_dt.label('session_created_dt'),
                    ...
                    TableSession.is_closed.label('session_is_closed'))
                                    .filter(TableSession.table_cd == table_cd,
                ...
                table_session = table_session_query.first()

                # Initialize `data_list` to store results and determine session status
                data_list = {}
                if table_session:
                    # Assign query results to `data_list`
                    data_list["session_created_dt"] = table_session.session_created_dt
                    data_list["session_amount"] = table_session.session_amount
                    data_list["session_is_open"] = table_session.session_is_open
                    data_list["session_is_closed"] = table_session.session_is_closed

                    # Add logic for determining `session_status`
                    if table_session.session_is_open == constants.YES:
                        session_status = "OPEN"
                    elif table_session.session_is_open == constants.NO:
            ...
            bill_dtl_query = (db.query(Table.cd, Table.nm.label("table_nm"), BillDtl, BillDtl.price,
                                    BillDtl.discount, Menu.nm.label("menu_nm"), Menu.img, Bill.user_nm)
            ...
            table_name_query = db.query(Table.nm).filter(Table.cd == table_cd).first()
            if table_name_query:
                res["table_nm"] = table_name_query.nm
            else:
                waiting_list = True
                waiting_query = (db.query(WaitingList.cd, WaitingList.nm.label("table_nm"), BillDtl,
                                        Menu.price, Menu.nm.label("menu_nm"), Menu.img, Bill.user_nm)
                                .join(Menu, Menu.cd == BillDtl.menu_cd)
                                .join(Bill, Bill.cd == BillDtl.bill_cd)
                ...
                if waiting_data:
                    res["table_nm"] = "Waiting List - " + waiting_data[0].table_nm if waiting_list else ""
                    listData = [
                        {
                            "cd": mdl.BillDtl.cd,
                            "bill_cd": mdl.BillDtl.bill_cd,
                        ...
                        for mdl in waiting_data if mdl.BillDtl.qty > 0
                    ]
    ...
    def getBillDtlByBillCd(self, request, db):
        ...
        try:
            ...
            if data:
                for mdl in data:
                    if mdl.BillDtl.qty > 0:
                        session_status = "CLOSED"  # Default status if none of the conditions match
            ...
            status = ""
            if data_bill:
                if data_bill.is_closed == constants.NO and data_bill.is_paid == constants.YES:
                    status = "EMPTY"
                elif data_bill.is_closed == constants.YES and data_bill.is_paid == constants.NO:
                    status = "CLOSED"
                elif data_bill.is_closed == constants.NO and data_bill.is_paid == constants.NO:
                    status = "OCCUPIED"
        ...
        try:
            ...
            for mdl in data:
                if mdl.BillDtl.qty > 0:
                    ...
                    price_discount = float(mdl.BillDtl.price) - float(mdl.BillDtl.discount)
    ...
    def bulkaddBillDetail(self, request, db):
        ...
        try:
            ...
            if billdata:
                ...
                query = query.filter(AppSetting.cd == 'printer')
                print_setting = query.first()
                print_name = print_setting.value
                ...
                printData["created_dt"] = dt.datetime.now()
                query = db.query(User.nm)
                query = query.filter(User.cd == request.orders[0].created_by)
                printData["created_by"] = query.first()
                ...
                printSelenium.printKitchenBill(printData,print_name)
            else:
                query = db.query(Bill)
                query = query.filter(Bill.cd == request.bill_cd)
