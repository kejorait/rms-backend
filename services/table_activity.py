...
class TableService:
    ...
    # Get All Table
    def getAllTable(self, request, db):
        jsonStr = {}
        try:
            # print("yes")
            ...
            # subquery = subquery.filter(Table.is_billiard == constants.YES)
    ...
    def getTableByCode(self, request, db):
        jsonStr = {}
    ...
    def getAllTableBilliard(self, request, db):
        jsonStr = {}
        try:
            # print("yes")
            # Define subquery to get the row number for each table.cd ordered by created_dt
